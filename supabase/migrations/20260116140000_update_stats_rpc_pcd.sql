...
BEGIN
  WITH dados AS (
    SELECT
      ...
      (SELECT r.resposta FROM respostas r WHERE r.user_expandido_id = u.id AND r.id_pergunta = 'eae93308-1e4d-4c67-9c53-c9abf6a31eaf' LIMIT 1) AS pcd
  ...
  pcd_stats AS (
    SELECT COALESCE(pcd, 'Não informado') as label, COUNT(*) as count
    FROM dados
    GROUP BY label
  ),
  total_count AS (
  ...
  SELECT jsonb_build_object(
      ...
      'pcd', (SELECT COALESCE(jsonb_object_agg(label, count), '{}'::jsonb) FROM pcd_stats)
  )
  INTO v_result;
