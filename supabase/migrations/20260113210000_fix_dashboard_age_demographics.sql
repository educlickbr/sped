...
BEGIN
    ...
    -- Idade (Calculated)
    -- IMPROVED: Handles YYYY-MM-DD and DD/MM/YYYY formats
    ...
    FROM (
        ...
        FROM (
            SELECT 
                EXTRACT(YEAR FROM (
                    CASE 
                        WHEN r.resposta ~ '^\d{4}-\d{2}-\d{2}$' THEN AGE(CURRENT_DATE, r.resposta::date)
                        WHEN r.resposta ~ '^\d{2}/\d{2}/\d{4}$' THEN AGE(CURRENT_DATE, to_date(r.resposta, 'DD/MM/YYYY'))
                        ELSE NULL
                    END
                )) as age
            FROM respostas r
