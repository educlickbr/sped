DROP FUNCTION IF EXISTS public.get_respostas_arquivos_area_v2(tipo_processo, tipo_area, tipo_candidatura, uuid, uuid);
...
--------------------------------------------------------------------------------
-- 2. NEW FUNCTION: get_respostas_arquivos_area_v2 (Consistent matching)
--------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_respostas_arquivos_area_v2(
...
BEGIN
  ...
  -- 2. Seleção de OBRIGATÓRIOS (Consistente com get_turmas_por_aluno_processos_v2)
  obrigatorias AS (
      SELECT
      ...
      WHERE 
          ...
          -- APENAS ARQUIVOS e LINKS (get_turmas só valida 'arquivo', mas aqui exibimos ambos)
          ...
          -- Se p_tipo_candidatura for passado, deve bater EXATO.
          AND (pdo.tipo_candidatura IS NOT DISTINCT FROM p_tipo_candidatura)
  ),
  ...
  unificado AS (
      ...
      LEFT JOIN LATERAL (
        ...
        -- Aceita apenas respostas:
        -- 1. Globais (id_turma IS NULL)
        -- 2. Desta turma específica (id_turma = p_id_turma)
