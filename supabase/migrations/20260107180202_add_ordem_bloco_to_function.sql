-- Migration to update the nxt_get_respostas_usuario_turma function to include ordem_bloco in the response and order by ordem_bloco and ordem

DROP FUNCTION IF EXISTS public.nxt_get_respostas_usuario_turma(uuid, uuid, tipo_processo, tipo_candidatura);
...
BEGIN
    ...
    fake AS (
        SELECT * FROM (
            VALUES
            (gen_random_uuid(), 'nome', 'Nome', 'texto', 'dados_pessoais'::bloco_pergunta, 1, NULL, false, 1, 36, false, NULL, v_user_nome, true),
            (gen_random_uuid(), 'sobrenome', 'Sobrenome', 'texto', 'dados_pessoais'::bloco_pergunta, 2, NULL, false, 1, 36, false, NULL, v_user_sobrenome, true),
            (gen_random_uuid(), 'email', 'E-mail', 'texto', 'dados_pessoais'::bloco_pergunta, 3, NULL, false, 2, 36, false, NULL, v_user_email, true)
    ...
    FROM (
        ...
        SELECT
            NULL, f.id_pergunta, f.bloco, f.ordem, f.ordem_bloco, false, f.largura, f.altura, false, NULL, NULL, false, NULL, p_tipo_processo, p_tipo_candidatura,
