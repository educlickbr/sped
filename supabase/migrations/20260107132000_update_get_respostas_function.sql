...
BEGIN
    ...
    WITH obrig AS (
        SELECT DISTINCT
            ...
            p.pergunta, p.label, p.tipo, p.opcoes, -- Added p.opcoes
            pdo.escopo
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'opcoes', x.opcoes, -- Added x.opcoes
            'uploading', false,
            'deleting', false,
            'load', false
        )
    ...
    FROM (
        ...
        SELECT
            ...
            f.pergunta, f.label, f.tipo, NULL::jsonb, NULL::escopo_processo, true, f.resposta, NULL, NULL -- Added NULL::jsonb for opcoes
        FROM fake f
    ) x;
