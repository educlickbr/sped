DROP FUNCTION IF EXISTS public.nxt_get_respostas_usuario_turma(
    uuid, uuid, tipo_processo, tipo_candidatura
);
...
BEGIN
    ...
    WITH obrig AS (
        SELECT DISTINCT
            ...
            pdo.tipo_processo, pdo.tipo_candidatura, pdo.ordem_bloco,
    ...
    fake AS (
        SELECT * FROM (
            VALUES
            (gen_random_uuid(), 'nome', 'Nome', 'texto', 'dados_pessoais'::bloco_pergunta, 1, false, 1, 36, false, NULL, v_user_nome, true, 1::integer),
            (gen_random_uuid(), 'sobrenome', 'Sobrenome', 'texto', 'dados_pessoais'::bloco_pergunta, 2, false, 1, 36, false, NULL, v_user_sobrenome, true, 2::integer),
            (gen_random_uuid(), 'email', 'E-mail', 'texto', 'dados_pessoais'::bloco_pergunta, 3, false, 2, 36, false, NULL, v_user_email, true, 3::integer)
        ) AS t(id_pergunta, pergunta, label, tipo, bloco, ordem, obrigatorio, largura, altura, depende, depende_de, resposta, artificial, ordem_bloco)
    )
    ...
    SELECT jsonb_agg(
        ...
        ORDER BY x.ordem_bloco, x.bloco, x.ordem
    )
    INTO v_perguntas
    FROM (
        SELECT 
        ...
        SELECT
            NULL, f.id_pergunta, f.bloco, f.ordem, false, f.largura, f.altura, false, NULL, NULL, false, NULL, p_tipo_processo, p_tipo_candidatura, f.ordem_bloco,
