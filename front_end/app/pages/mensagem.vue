...
const icon = computed(() => {
    switch(store.statusMessage.type) {
        case 'success':
            return `<svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 text-success" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>`
        case 'error':
...
<div class="min-h-screen flex items-center justify-center bg-[#0a0a0c] p-6 font-sans relative overflow-hidden">
    ...
    <div class="w-full max-w-md relative z-10">
        <div class="bg-div-15 backdrop-blur-2xl border border-white/5 rounded-[2.5rem] p-8 md:p-12 shadow-2xl text-center flex flex-col items-center gap-6">
            
            <div class="w-24 h-24 rounded-full bg-white/5 flex items-center justify-center mb-2" v-html="icon"></div>
            ...
            <button 
                @click="handleAction"
                class="mt-4 w-full bg-primary text-white font-black py-4 rounded-xl text-sm uppercase tracking-widest shadow-lg shadow-primary/20 hover:bg-[#b81151] hover:shadow-primary/40 hover:-translate-y-0.5 transition-all active:scale-[0.98]"
            >
