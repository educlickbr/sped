-- 1. Calcular o "Best Match" (AGORA COM FILTRO DE ESCOPO PRIMEIRO)
DROP TABLE IF EXISTS tmp_best_match_ranked;

-- Criação explicita da tabela para evitar erros de parser
CREATE TEMP TABLE tmp_best_match_ranked (
    id_resposta uuid,
    user_expandido_id uuid,
    id_pergunta uuid,
    id_turma_atual uuid,
    id_turma_nova uuid,
    area_turma_nova text,
    diff_tempo double precision,
    rn_source bigint
);

-- Inserção dos dados
INSERT INTO tmp_best_match_ranked
...
-- Candidatos Validos: Combinação de processos que ACEITAM a pergunta
CandidatosValidos AS (
    SELECT 
        r.id AS id_resposta,
        r.user_expandido_id,
        r.id_pergunta,
    ...
    JOIN public.turmas t ON proc.turma_id = t.id
    WHERE 
      -- O filtro de Scopo agora é aplicado AQUI, antes de escolher o melhor tempo
      EXISTS (
...
BestMatch AS (
    -- Agora pegamos o melhor tempo DENTRE os válidos
    SELECT DISTINCT ON (cv.id_resposta)
        cv.*
    FROM CandidatosValidos cv
    ORDER BY cv.id_resposta, cv.diff_tempo ASC
)
SELECT 
    id_resposta,
    user_expandido_id,
...
FROM BestMatch
-- Só queremos quem de fato vai mudar de turma
WHERE id_turma_atual IS DISTINCT FROM id_turma_nova;
