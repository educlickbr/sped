SELECT
    ...
    ue.sobrenome,
    ue.email,
    t.nome_curso as turma_matriculada,
    -- Bloco: None | Ordem: None
    (SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = '74545877-be62-49e1-8354-8fd004c57a61' ORDER BY r.criado_em DESC LIMIT 1) as "Telefone do Contato de Emergência"
    -- Bloco: None | Ordem: None
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = '289e962e-da61-4cf3-9810-7ece9de8f19f' ORDER BY r.criado_em DESC LIMIT 1) as "Nome do Contato de Emergência"
    -- Bloco: None | Ordem: None
    ...
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = '5747a06b-2385-4920-af8a-48f8ec870518' ORDER BY r.criado_em DESC LIMIT 1) as "Nome Artístico"
    ...
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = 'd42f73a8-0a61-4f6a-acd1-767b06bd612e' ORDER BY r.criado_em DESC LIMIT 1) as "Caso não receba bolsa, você terá condições de manter os seus estudos?"
    ...
    -- Bloco: dados_pessoais | Ordem: 13
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = '18ad34c5-f9f1-4f0c-82bb-062977b4800c' ORDER BY r.criado_em DESC LIMIT 1) as "Você possui aptidão física para participar de atividades  e práticas corporais, como as aulas de dança? "
    -- Bloco: dados_pessoais | Ordem: 14
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = '4fda0813-526b-410c-8a21-819384bf341a' ORDER BY r.criado_em DESC LIMIT 1) as "Autorização para Contato Físico   Durante as aulas de dança, ensaios e  atividades formativas, poderá haver contato físico orientado, como correções de postura, condução de movimentos e interações coreográficas. Essas práticas fazem parte dos processos pedagógicos e artísticos da escola.  Você autoriza a participação em atividades que envolvam esse tipo de contato físico orientado, dentro do contexto pedagógico da escola? "
    ...
    -- Bloco: pcd | Ordem: 3
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = '25ea2ab7-c0da-441d-8a10-bf724523c5a1' ORDER BY r.criado_em DESC LIMIT 1) as "Precisa de algum recurso de acessibilidade? Em caso positivo, qual?"
    -- Bloco: pcd | Ordem: 4
    ...
    ,(SELECT r.resposta FROM public.respostas r WHERE (r.user_expandido_id = ue.id OR r.id_usuario = ue.user_id) AND r.id_pergunta = 'cc164282-0822-4ba1-9dc1-c0db537ebb77' ORDER BY r.criado_em DESC LIMIT 1) as "Ao realizar esta inscrição o(a) candidato(a) ou o(a) responsável legal do candidato(a) da pessoa com idade menor de 18 anos declara que leu e concorda com os termos do Edital para este processo seletivo, e que está apto(a) a realizar a prova prática, acatando integralmente as regras estabelecidas, não cabendo qualquer recurso posterior."
FROM
    public.matriculas m
JOIN
...
WHERE
    ...
    AND public.normalizar_texto(t.area_curso) = 'regulares'
