-- FIX: Robust Area Matching for get_respostas_arquivos_area
-- Relies on normalizar_texto to match pdo.id_area and p_area input
-- even if there are subtle differences (case, accents) in the Enum conversion/usage.
...
BEGIN
  ...
  obrigatorias AS (
      ...
      WHERE 
          ...
          -- área ou turma (ROBUST FIX)
          AND (
               (
                 pdo.escopo = 'area'  
                 -- FIX: Comparison robusta que ignora case/acentos
