CREATE OR REPLACE FUNCTION public.nxt_get_candidatos_processo_turma_v2(p_id_turma uuid DEFAULT NULL::uuid, p_pagina integer DEFAULT 1, p_limite integer DEFAULT 20, p_tipo_candidatura tipo_candidatura DEFAULT NULL::tipo_candidatura, p_busca text DEFAULT NULL::text, p_filtros jsonb DEFAULT '[]'::jsonb, p_pcd text DEFAULT NULL::text, p_laudo boolean DEFAULT NULL::boolean, p_ordenar_por text DEFAULT 'nome_completo'::text, p_ordenar_como text DEFAULT 'ASC'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
    ...
    'itens', COALESCE(
      jsonb_agg(
        jsonb_build_object(
          ...
          -- REMOVED: avaliacao (simplifying response)
        )
      ),
      '[]'::jsonb
    )
