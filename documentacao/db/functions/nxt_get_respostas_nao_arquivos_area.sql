...
BEGIN
    ...
    WITH obrig AS (
        SELECT DISTINCT
            ...
            pdo.ordem, 
            pdo.obrigatorio, 
            pdo.largura, -- USING INTEGER COLUMN AS REQUESTED
            pdo.altura,  -- USING INTEGER COLUMN AS REQUESTED
            pdo.depende, 
            pdo.depende_de, 
            pdo.valor_depende, 
        ...
        WHERE 
            ...
            -- Filter OUT 'arquivo' and 'link' types (we only want 'Dados')
    ...
    -- 5) ARTIFICIAL FIELDS (Mocked questions that don't exist in DB but are needed in UI)
    fake AS (
        SELECT * FROM (
            VALUES
